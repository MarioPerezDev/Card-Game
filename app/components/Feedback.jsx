...
render(){
    ...
    return (
        <Col className={"d-flex justify-content-center"} xs={12} md={4} key={i}><Card cardClassName={"allyCard " + classchosen} key={i} tracking={this.props.tracking} dispatchable={true} activePowerUp={this.props.game.powerUp} number={i+1} name={card.name} power={card.power} image={card.image} enemypower={round.enemyCard.power} currentPowerUp={this.props.game.powerUp} powerinfo={card.feedback} dispatch = {this.props.dispatch} configs = {this.props.configs}/></Col>
    )}
    ));
    return (
